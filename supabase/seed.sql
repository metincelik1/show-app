...
('turkish-immigrants','Turkish Immigrants',  'Türk Göçmenler',       'Bir Konsolosluk sırası diyaloğu ya da memleketten gelen bir akrabanın yazdığı sitem dolu WhatsApp mesajı — aşırı ciddi başlayıp giderek absürd bir hal alıyor.', 'tr');

-- SUBURB (user-provided questions)
...
insert into questions (theme_id, order_num, text) select id, 7,  'Complete the phrase: "You know you are in the suburbs when you see a _____."' from themes where slug='suburb';
...
-- MUSLIMS ABROAD (user-provided questions)
...
-- FATHER'S DAY (user-provided questions)
...
insert into questions (theme_id, order_num, text) select id, 10, 'Complete the sentence: "A real man always knows how to _____."' from themes where slug='fathers-day';

-- IMMIGRANTS (user-provided questions)
...
-- CORPORATE (user-provided questions)
...
insert into questions (theme_id, order_num, text) select id, 3,  'What is a completely valid reason to turn your camera off during a Zoom/Teams meeting?' from themes where slug='corporate';
...
-- TURKISH IMMIGRANTS (user-provided questions — in Turkish)
...
insert into questions (theme_id, order_num, text) select id, 2,  'Amerikalıların / yabancıların bir türlü anlayamadığı, açıklamaktan yorulduğunuz Türk adeti nedir?' from themes where slug='turkish-immigrants';
...
insert into questions (theme_id, order_num, text) select id, 7,  'Evinize gelen bir yabancı misafirin gördüğünde şok olduğu ev eşyası nedir? (Örn: dantel, terlik, klozetteki maşrapa)' from themes where slug='turkish-immigrants';
insert into questions (theme_id, order_num, text) select id, 8,  'Türkiye''deki akrabaların "Siz orada kesin _____ yapıyorsunuzdur" diye düşündüğü en büyük yanlış anlama nedir?' from themes where slug='turkish-immigrants';
insert into questions (theme_id, order_num, text) select id, 9,  'Buranın kültürüne ayak uydurmaya çalışırken yaptığınız en büyük pot veya çam devirme anısı nedir?' from themes where slug='turkish-immigrants';
...
-- GENERAL (user-provided questions)
...
insert into questions (theme_id, order_num, text) select id, 10, 'Complete the phrase: "You should never trust a person who _____."' from themes where slug='general';
