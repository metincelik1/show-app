...
export async function POST(req: NextRequest) {
  const { showId, bestAnswers: providedBestAnswers } = await req.json();
  ...
  let crowdFavorites: Record<number, string[]> = {};

  // Use pre-ranked best answers if Crowd Favorites was already run — avoids re-ranking and duplicate reveals
  if (providedBestAnswers && Object.keys(providedBestAnswers).length > 0) {
    picks = orderedQuestions.map(q => ({
      i: q.idx,
      best: providedBestAnswers[q.qid] ?? q.answers[0] ?? "",
      second: "",
      third: "",
    }));
    // crowdFavorites already shown — don't return them again
  } else {
